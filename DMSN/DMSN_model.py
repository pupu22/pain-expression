...
class P3D(nn.Module):
    ...
    def __init__(self, block, layers, modality='RGB',
        shortcut_type='B', num_classes=6,dropout=0.5,ST_struc=('A','B','C')):
        self.inplanes = 64
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    if pretrained==True:
        if modality=='RGB':
            pretrained_file='/home/cike/pythonGC/p3d_rgb_199.checkpoint.pth.tar'
        elif modality=='Flow':
...
if __name__ == '__main__':
    ...
    model = P3D199(pretrained=True,num_classes=6)
    model = model.cuda()
    data=torch.autograd.Variable(torch.rand(8,3,16,160,160)).cuda()   # if modality=='Flow', please change the 2nd dimension 3==>2
    out=model(data)
    print (out.size(),out)
