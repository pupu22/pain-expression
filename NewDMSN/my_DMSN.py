...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplane, midplane, StrideS, PaddingS, StrideT, PaddingT, downsample=None, st_struc='A',
        ...
        # 这里stage1输入了16，one_plane为32
        one_plane = midplane * 2

        # 与原本的baseline相比，修改了A块，增加了时间卷积的层数，减少了分支数
        if self.st_struc == 'A':
            ...
            self.conv6 = conv_S(one_plane, one_plane, stride=StrideS, padding=PaddingS)
            ...
            self.convAdd = conv_T(one_plane, one_plane, stride=(1, 1, 1), padding=(0, 0, 1))
            self.bnAdd = nn.BatchNorm3d(one_plane)
    ...
    def make_layer(self, block, midplane, block_num, shortcut_type, stride=1):
        ...
        if shortcut_type == 1:
            ...
            '''
                -----------------------------stage2-------------------------------------
                A-B-C-A
                A块修改图像大小与通道数，并在A模块增加downsample，即直连部分增加一个卷积，改变通道数和大小
                A块的stride与padding修改图像大小，number修改通道数
            '''
            downsample = nn.Sequential(
...
# resnet =  my_DMSN(Bottleneck, [3, 4, 6, 3])
# resnet = resnet.cuda()
